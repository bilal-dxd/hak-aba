...
// vérifie si la clé 'user_id' existe dans les cookies.
...
<section class="orders">
   ...
   <?php
      ...
      // Vérifier s'il y a des commandes disponibles pour cet utilisateur

      if($select_orders->rowCount() > 0){
         while($fetch_order = $select_orders->fetch(PDO::FETCH_ASSOC)){
            $select_product = $conn->prepare("SELECT * FROM `products` WHERE id = ?");
            $select_product->execute([$fetch_order['product_id']]);
            // Vérifier s'il y a un produit disponible avec cet identifiant
   ...
   <div class="box" <?php if($fetch_order['status'] == 'canceled'){echo 'style="border:.2rem solid red";';}; ?>>
      <a href="view_order.php?get_id=<?= $fetch_order['id']; ?>">
         ...
         <p class="price"> <?= $fetch_order['price']; ?> MAD x <?= $fetch_order['qty']; ?></p>
